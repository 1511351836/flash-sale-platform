...
export class PaymentDetailsWrapper extends React.Component{
  render() {
    const {groupId, userId, address} = this.props.location.state || {};
    console.log('detail screen wrapper props: ', groupId);
    console.log('detail screen wrapper userId: ', userId);
    return(
        <PaymentDetailsView groupId={groupId} userId={userId} address={address}/>
    );
  }
}

const PaymentDetailsView = ({groupId, userId, address}) => {
  ...
  const onPressBuy = () => {
    ...
    if (address.addressId === 0 && addressId === 0) {
      ...
      //   placement: 'top',
      // });
      message.warning('新地址还没有保存编辑，保存后重试！');
    } else {
            ...
            <HStack>
              ...
              <Button
                ...
                onPress={() => {
                  ...
                  message.warning("目前web端不支持从历史地址中进行选取，请下载APP获取更加体验！");
                }}>
...
export default withRouter(PaymentDetailsWrapper);
